...
export default function useTabStyles(props: TabProps) {
  ...
  const styles = [];
  styles.push(
    tw`-mb-px border-0 border-b-2 m-0 border-transparent px-4 py-3 text-gray-500 focus:outline-none bg-transparent`
  );
  ...
  if (selected) {
    styles.push(
      `color: ${theme.color.primary}; border-bottom-color: ${theme.color.primary};`
    );
  }
